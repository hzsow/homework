...
const TransferEachOtherForm = React.memo((props) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
        <Field label="Счет списания" name="currentAccount" component={ASelect} defaultValue={accounts.length > 0 ? accounts[0].accountNumber: ""} placeholder="" >
            { 
        ...
        <Field label="Счет зачисления" name="receiverAccount" component={ASelect} defaultValue={accounts.length > 0 ? accounts[0].accountNumber: ""}  placeholder="" >
            { 
              accounts.map((element) => {
                return <Option key={element.accountNumber} value={element.accountNumber}>{element.accountNumber}</Option>
              }) 
